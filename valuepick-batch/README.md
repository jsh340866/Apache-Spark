...
```
              ...
              ┌────────────┴────────────┐
      ┌───────┴───────┐         ┌───────┴───────┐
      │ spark-worker-1 │         │ spark-worker-2 │   각 2코어 / 3GB (환경별 상이, docs/ENVIRONMENT.md 참고)
...
- **`portfolio_size` 후보**: `10 / 30 / 50 / 100 / 300`. 처음엔 "사실상 무제한"(9999)으로 대조군을 만들려 했으나, 시점마다 조건 통과 종목 전부를 그대로 담아 중간 결과 부피가 기존 대비 5~6배로 커지면서 드라이버 OOM이 실제로 발생해 300으로 낮췄다(`conf/generate_strategies.py` 주석 참고). 여전히 10/30/50/100보다는 훨씬 느슨한 선별이라 "선별 강도가 약해질수록 성과가 어떻게 변하는가"는 볼 수 있다
